...
import { crawlerMultiBarLogger, updateProgressBar, updateTotalProgressBar } from "./progressBar.js";
import { ensureDirectoryExistence, toArray } from "./utils.js";

export function getPageHtml(page: Page, selector = "body", excludeSelectors: string | string[] = "") {
	...
	return page.evaluate(
		({ selector, excludeSelectors }) => {
			const clonedDocument = cloneDocumentAndRemoveSelectorsContent(document, excludeSelectors);
			 ...
			 */
			function cloneDocumentAndRemoveSelectorsContent(doc: Document, excludeSelectors: string[]): Document {
				const clonedDocument = doc.cloneNode(true) as Document;
				excludeSelectors.forEach(selector => {
					if (selector) {
						clonedDocument.querySelectorAll(selector).forEach(el => el.remove());
					}
				});
			 ...
			 */
			function getTextContentBySelector(doc: Document, selector: string): string {
				if (isXPath(selector)) {
			...
			function evaluateXPathOnDocument(doc: Document, xpath: string): string {
				const result = doc.evaluate(xpath, doc, null, XPathResult.ANY_TYPE, null).iterateNext();
...
export async function crawl(inputConfig: ConfigInput, progressBarId: number = 0) {
	...
	if (process.env.NO_CRAWL !== "true") {
		...
		const crawler = new PlaywrightCrawler({
			...
			async requestHandler({ request, page, enqueueLinks /*, log*/ }) {
				...
				updateTotalProgressBar(progressBarId, Math.min(requestQueue.getTotalCount(), config.maxPagesToCrawl));
				...
				const html = await getPageHtml(page, config.selector, config.excludeSelectors);
