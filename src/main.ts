...
import { createProgressBar, getLogs, logMessage, multiBar } from "./progressBar.js";
import { parallelAsyncMap, toArray } from "./utils.js";

const main = async () => {
	const allConfigs = toArray(defaultConfig).map(config => configSchema.parse(config));
	allConfigs.forEach((config, index) => createProgressBar(config.maxPagesToCrawl ?? 1000, index, config));

	await parallelAsyncMap(allConfigs, async (config: ConfigInput, index: number) => {
