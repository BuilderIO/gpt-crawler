...
export const multiBar: MultiBar = new cliProgress.MultiBar(
	{
		...
		forceRedraw: true,
		linewrap: true,
		format: " {bar} | {name} | {value}/{total} (L: {limit}, F: {found}) | ETA: {eta}s | {url} ",
	},
...
export function createProgressBar(totalValue: number, id: number, config: Config): void {
...
class ProgressBarLogger extends Logger {
	...
	override _log(level: LogLevel, message: string, data = {}, exception = undefined, opts = {}) {
